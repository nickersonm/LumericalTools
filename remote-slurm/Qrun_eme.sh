...
#   Input: lms file with EME simulation
#   Set REFRESH=1 to refresh lms
...
NODES=4  # Does not scale so well to large numbers
QRUN="/usr/bin/srun --exclude=node19 --chdir=""$EXECDIR"" --mpi=pmi2 --nodes=$NODES --cpus-per-task=$THREADS --time=10:00:00 --mem=12G -p batch --job-name=emeEng"
...
ENG="/sw/cnsi/lumerical-2021R1/bin/eme-engine"
