...
DEVICESUB="~/lumerical/MQ_queue_device.sh"
MODESUB="~/lumerical/MQ_queue_mode.sh"
...
for file in "$@"; do {
    ...
    # Submit to local pueue queue for DEVICE if no result file exists
    [ ! -f "${INPUT/.lsf/_CHARGE.mat}" ] && pueue add -g local "$DEVICESUB" "$INPUT"
    
    # Submit to local pueue queue for MODE if no result file exists
    [ ! -f "${INPUT/.lsf/_MODE.mat}" ] && pueue add -g local "$MODESUB" "$INPUT"
}
done
